...
The container represents \textit{domain product} (defined in section \ref{sec:domain-product}). To remind, domain products serves as an encapsulation for multiple abstract values representing the same program variable. And similarly is defined the first container's responsibility:
...
Also the container has to fulfil a part of the \textit{reduced product} (defined in section \ref{sec:reduced-cardinality-product}) over-approximation role -- therefore the following responsibility has to be implemented:
